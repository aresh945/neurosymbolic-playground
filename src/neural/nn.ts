...
/*
 * VENDORED from TensorFlow Playground (https://github.com/tensorflow/playground)
 * File: src/nn.ts — the neural network engine.
 * Kept faithful so the "expand the neural block" view behaves identically to
 * the original TensorFlow Playground. Only formatting/comments touched.
 */
...
export class Node {
   ...
   * the last update. This derivative equals dE/db where b is the node's bias.
   */
  accInputDer = 0;
  /**
 ...
 * derivatives with respect to each node, and each weight in the network.
 */
